## ATTRIBUTION — Magyar Diceware Szószedetek

Project:   Magyar Diceware Szószedetek (Hungarian Diceware Wordlists)
Author:    CarryPass / Rácz Zoltán
Copyright: (c) 2026 CarryPass / Rácz Zoltán
License:   Creative Commons Attribution-ShareAlike 4.0 International
           (CC BY-SA 4.0)
           See LICENSE for the full legal text.
           https://creativecommons.org/licenses/by-sa/4.0/

Online generator:  https://kockajelszo.hu
Source repository: https://github.com/racz-zoltan/hungarian-diceware


## SOURCE MATERIAL

The wordlists in this project were curated from the
Magyar Wikiszótár (Hungarian Wiktionary):

  https://hu.wiktionary.org/wiki/Kateg%C3%B3ria:Magyar_szavak_list%C3%A1ja

The Magyar Wikiszótár is licensed under
Creative Commons Attribution-ShareAlike 4.0 International
(CC BY-SA 4.0). This derivative work therefore inherits the
ShareAlike obligation: any redistribution or adaptation of
these wordlists must be licensed under the same CC BY-SA 4.0
license (or a later compatible version listed at
https://creativecommons.org/compatiblelicenses).


## HOW TO ATTRIBUTE THIS WORK

When you reuse, redistribute, or build upon these wordlists,
please include the attribution below in any reasonable manner
appropriate to your medium (e.g. in a README, a credits page,
an "About" dialog, or a printed colophon).


## Recommended attribution text (English)


  "Magyar Diceware Szószedetek" by CarryPass / Rácz Zoltán,
  licensed under CC BY-SA 4.0
  (https://creativecommons.org/licenses/by-sa/4.0/).
  Source: https://github.com/racz-zoltan/hungarian-diceware

  Word source: Magyar Wikiszótár (Hungarian Wiktionary),
  https://hu.wiktionary.org/wiki/Kateg%C3%B3ria:Magyar_szavak_list%C3%A1ja
  also licensed under CC BY-SA 4.0.


## Ajánlott attribúciós szöveg (magyar)

  „Magyar Diceware Szószedetek", készítette CarryPass / Rácz Zoltán,
  CC BY-SA 4.0 licenc alatt
  (https://creativecommons.org/licenses/by-sa/4.0/deed.hu).
  Forrás: https://github.com/racz-zoltan/hungarian-diceware

  A szavak forrása: Magyar Wikiszótár,
  https://hu.wiktionary.org/wiki/Kateg%C3%B3ria:Magyar_szavak_list%C3%A1ja
  szintén CC BY-SA 4.0 alatt.


## WHAT THE LICENSE REQUIRES

CC BY-SA 4.0 grants you the right to:

  - copy, redistribute, and use the material in any medium or
    format, including commercially;
  - remix, transform, and build upon it.

In exchange you must:

  - give appropriate credit (see "Recommended attribution text"
    above), provide a link to the license, and indicate if you
    made changes;
  - distribute any derivative works under the same CC BY-SA 4.0
    license (or a later compatible version);
  - not apply legal terms or technological measures that legally
    restrict others from doing anything the license permits.

If you make changes to the wordlists (additions, removals,
reorderings, format conversions), please note this in your
distribution, for example:

  "Modified from the original 'Magyar Diceware Szószedetek' by
   CarryPass / Rácz Zoltán. Changes: <brief description>."


## ATTRIBUTION TO REPORT INAPPROPRIATE WORDS

If you find a word in these lists that is offensive, slurring,
or otherwise inappropriate for a passphrase generator, please
report it to:

  carrypass-info [at] proton.me

It will be removed in the next revision.
